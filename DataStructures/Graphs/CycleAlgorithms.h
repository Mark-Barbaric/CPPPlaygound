...
{
	bool containsDirectedCycleUtil(int u, std::vector<bool>& visited, const std::vector<std::list<int>>& adj)
	{
		if (visited[u])
			return true;
	...
	bool containsUndirectedCycleUtil(int parent, int u, std::vector<bool>& visited, const std::vector<std::list<int>>& adj)
	{
		visited[u] = true;
	...
	bool containsCycle(const Graph& graph, bool directed)
	{
